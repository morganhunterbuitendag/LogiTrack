...
app.get('/api/distances', async (req,res)=>{
  try{
    const [storedProducers, rootProducers, processors] = await Promise.all([
      readArray('producers.json'),
      fs.readFile(path.join(process.cwd(),'producers.json'),'utf8').then(JSON.parse).catch(()=>[]),
      fs.readFile(path.join(process.cwd(),'processors.json'),'utf8').then(JSON.parse).catch(()=>[])
    ]);
    const producers = storedProducers.length ? storedProducers : rootProducers;
